...
// Generated once at module load (client-only module, ssr:false).
function buildField() {
  ...
  return { positions, colors, seeds, base: positions.slice() };
}
...
function Field() {
  ...
  useFrame(({ clock }) => {
    ...
    for (let i = 0; i < COUNT; i++) {
      ...
      arr[j] = FIELD.base[j] + Math.sin(t * 0.25 + s) * amp;
      arr[j + 1] = FIELD.base[j + 1] + Math.cos(t * 0.2 + s * 1.3) * amp;
    }
...
export default function HeroScene() {
  const [ok] = useState(canRenderScene);
  if (!ok) return null;
  return (
    <Canvas
